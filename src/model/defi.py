...
class Strategy:
    ...
    def execute(self, mode, **kwargs):
        if mode == self.SIMPLE:
            ...
            def update_last_transactions_list(adr, last_transactions: dict, transaction_hash):
                last_transactions[adr] = transaction_hash
                (pathlib.Path(__file__).parent.parent / "resources" / "last_transactions.json").write_text(
                    data=json.dumps(last_transactions), encoding="UTF-8")
    ...
    def check_and_set_allowance(self, web3, wallet: LocalAccount, token_address, approval_address, amount, gas):
        ...
        if token_address != ETH_ADDRESS:  # ETH
            ...
            allowance = erc20.functions.allowance(wallet.address, approval_address).call()
            ...
            if allowance < amount:
                txn = {
                    ...
                    "data": erc20.encodeABI(fn_name="approve", args=[approval_address, amount]),
                }
